...
public class GraspingLogicStick : MonoBehaviour
{
    ...
    {
        if (other.gameObject.name.Contains("rThDistal") || other.gameObject.name.Contains("rIndDistal"))
        {
            collision++;
        }
    ...
    void Update()
    {
        ...
        /*else
        {
            ...
            norm_diff_stk = distance.magnitude;
            //arbiter.GetMovementState() == vMPLMovementArbiter.MOVEMENT_STATE_CYLINDER_GRASP
            //&&


            if (angles[1] > 60.0 && angles[5] > 60.0 && angles[9] > 60.0 && angles[13] > 60.0 && angles[18] > 60.0 && angle_diff <= GRASP_ANGLE_THRESHOLD && norm_diff_stk <= GRASP_DIST_THRESHOLD)
            {
                GraspingStick = true;
